...
begin
if ((clAccessLevel=3) OR (clAccessLevel=0)) OR ((crFlag=true) AND (clientData.InTime<>'_')) then
...
begin
 if ((clAccessLevel=2) OR (clAccessLevel=0)) OR ((crFlag=true) AND (clientData.FactTime<>'_')) then
...
begin
 if ((clAccessLevel=3) OR (clAccessLevel=0)) OR ((crFlag=true) AND (clientData.OutDate<>'_')) then
  clientData.OutDate:=DateToStr(dtpOutDate.Date)
 else
  clientData.OutDate:='_';
end;
...
// clientDS.DataSet:=clientQuery;
 ...
 if clientQuery.RecordCount<>0 then
  begin
    clientQuery.First;
    clientData.InDate:=clientQuery.FieldByName('InDate').AsString;
    if clientData.InDate<>'_' then dtpInDate.Date:=StrToDate(clientData.InDate);
    clientData.FactTime:=clientQuery.FieldByName('FactTime').AsString;
    if clientData.FactTime<>'_' then dtpFactTime.Time:=StrToTime(clientQuery.FieldByName('FactTime').AsString);
    clientData.InTime:=clientQuery.FieldByName('InTime').AsString;
    if clientData.InTime<>'_' then dtpInTime.Text:=clientData.InTime;
    ...
    clientData.OutDate:=clientQuery.FieldByName('OutDate').AsString;
    if clientData.OutDate<>'_' then dtpOutDate.Date:=StrToDate(clientData.OutDate);
...
begin
 Caption:=Caption+': '+IntToStr(clAccessLevel);
 if crFlag then
  begin
   OpenClientCard(clID)
  end
...
try
 ...
 clientData.FactWeight:=clientData.FactPlotn*clientData.FactVolume;
 leFactWeight.Text:=FloatToStrF(clientData.FactWeight, ffFixed, 5, 0);
except
...
begin
 ...
 1:begin   //4-5
   dtpInDate.Enabled:=false;//1
   dtpOutDate.Enabled:=false;//2
   dtpInTime.Enabled:=false;//3
   cbCustomer.Enabled:=true;//4
   cbMaterial.Enabled:=true;//5
   leVolume.Enabled:=true;//6
   leDriver.Enabled:=true;//7
   cbTanks.Enabled:=false;//8
   leFactVolume.Enabled:=false;//9
   lePlotn.Enabled:=false;//10
   leFactTemper.Enabled:=false;//11
   leFactWeight.Enabled:=false;//12
   dtpFactTime.Enabled:=false;//13
   leDocuments.Enabled:=false;//14
   leDocuments.Visible:=true;
   leTonnaPrice.Enabled:=true;
   mNotes.Enabled:=false;//15
   if not clientData.Checked then cbChecked.Enabled:=true;
   actionEdit.Enabled:=true;
   actionSave.Enabled:=true;
 end;
 2:begin  //9-13
   ...
   cbCustomer.Enabled:=true;//4
   cbMaterial.Enabled:=true;//5
   leVolume.Enabled:=false;//6
   leDriver.Enabled:=false;//7
   cbTanks.Enabled:=false;//8
   leFactVolume.Enabled:=true;//9
   lePlotn.Enabled:=true;//10
   leFactTemper.Enabled:=true;//11
   leFactWeight.Enabled:=true;//12
   dtpFactTime.Enabled:=true;//13
   leDocuments.Enabled:=false;//14
   leDocuments.Visible:=false;
 ...
 3:begin   //1-3,6,7,15
   dtpInDate.Enabled:=true;//1
   dtpOutDate.Enabled:=true;//2
   dtpInTime.Enabled:=true;//3
   cbCustomer.Enabled:=false;//4
   cbMaterial.Enabled:=false;//5
   leVolume.Enabled:=true;//6
   leDriver.Enabled:=true;//7
   cbTanks.Enabled:=false;//8
   leFactVolume.Enabled:=false;//9
...
try
   ...
   if ((clAccessLevel=4) OR (clAccessLevel=0) OR (clAccessLevel=1)) AND (clientData.Documents<>0.0) then
    begin
