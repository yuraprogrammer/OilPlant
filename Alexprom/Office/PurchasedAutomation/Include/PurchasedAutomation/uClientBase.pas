...
uses
  ...
  ppViewr, ppVar, Excel2000, OleServer, ToolWin, ActnMan, ActnCtrls,
  XPStyleActnCtrls;
...
type
  TfrmClientBase = class(TForm)
    ...
    ADOQuery2FactTime: TWideStringField;
    ADOQuery2Checked: TBooleanField;
    ppDBText36: TppDBText;
    ADOQuery3Summa: TBCDField;
    ADOQuery3percentratio: TBCDField;
    ppDBText37: TppDBText;
    ADOQuery3TonnaPrice: TBCDField;
...
begin
 stmtSummary:='select sum(Volume) as volume,'+
          		 'sum(FactVolume) as factvolume,'+
          		 'sum(FactWeight) as factweight,'+
               'sum(Weight) as weight,'+
		           'sum(ShortageWeight) as shortageweight,'+
               'cast((sum(Summa)/sum(Weight))*1000-(sum(Summa)/sum(FactWeight))*1000 AS decimal(10, 2)) as shortagemoney,'+
               'sum(Summa) as Summa,'+
               'cast((sum(Summa)/sum(Weight))*1000 AS decimal(10, 2)) as TonnaPrice, '+
               'cast(((sum(FactWeight)-sum(Weight))/sum(Weight))*100 as decimal(10, 2)) as percentratio '+
...
begin
 frmClientCard:=TfrmClientCard.Create(Self);
 frmClientCard.crFlag:=false;
 frmClientCard.clAccessLevel:=clAccessLevel;
...
if ReportTabs.TabIndex=1 then
 begin
  ...
  stmtSummary:='select sum(Volume) as volume,'+
          		 'cast(sum(FactVolume) as decimal(10,0)) as factvolume,'+
          		 'sum(FactWeight) as factweight,'+
               'sum(Weight) as weight,'+
		           'sum(ShortageWeight) as shortageweight,'+
               'cast((sum(Summa)/sum(FactWeight))*1000-(sum(Summa)/sum(Weight))*1000 AS decimal(10, 2)) as shortagemoney,'+
               'sum(Summa) as Summa,'+
               'cast((sum(Summa)/sum(Weight))*1000 AS decimal(10, 2)) as TonnaPrice, '+
               'cast(((sum(FactWeight)-sum(Weight))/sum(Weight))*100 as decimal(10, 2)) as percentratio '+
               'from dbo.apViewReportIncomingTMC where ';
...
begin
if Item.SubItems[12]<>'0' then
 if Item.SubItems[14]<>'True' then
   lvList.Canvas.Brush.Color:=clGreen
 else
   lvList.Canvas.Brush.Color:=clWhite
else
...
begin
 if ReportTabs.TabIndex=0 then
  begin
    stmt:='select * from dbo.apViewIncomingTMC where Checked='+'''False''';
    actionExport.Enabled:=False;
  end;
 if ReportTabs.TabIndex=1 then
   begin
    if cbDateFilterUse.Checked then DateFilter.ItemIndex:=0;
    if ((clAccessLevel=0) OR (clAccessLevel=1)) then
     begin
      stmt:='select * from dbo.apViewReportIncomingTMC where Checked between '+'''False'''+' AND '+'''True''';
      actionExport.Enabled:=True;
     end
    else
     begin
      MessageDlg('У вас нет прав для просмотра данной страницы!!!', mtError, [mbOk], 0);
      Exit;
      //ReportTabs.TabIndex:=0;
      //ReportTabsChange(nil);
     end;
   end;
 BuildStatement(stmt);
end;
...
procedure TfrmClientBase.reportViewCustomDrawItem(Sender: TCustomListView;
  Item: TListItem; State: TCustomDrawState; var DefaultDraw: Boolean);
begin
if Item.SubItems[13]='False' then
...
begin
 SQLStmt:=SQL;
 summary:='select sum(Volume) as volume,'+
          		 'cast(sum(FactVolume) as decimal(10,0)) as factvolume,'+
          		 'sum(FactWeight) as factweight,'+
               'sum(Weight) as weight,'+
		           'sum(ShortageWeight) as shortageweight,'+
 ...
 if j<>0 then //если выбраны контрагенты
  begin
   ...
   if customersID.Count=1 then
    begin
      SQLStmt:=SQLStmt+' AND '+customersID.Strings[k];
      summary:=summary+' AND '+customersID.Strings[k];
    end
   else
    begin
     SQLStmt:=SQLStmt+' AND (';
 ...
 if i<>0 then //выбрана ТМЦ
  begin
   ...
   if productsID.Count=1 then
    begin
     SQLStmt:=SQLStmt+' AND '+productsID.Strings[k];
     summary:=summary+' AND '+productsID.Strings[k];
    end
   else
    begin
     SQLStmt:=SQLStmt+' AND (';
 ...
 if useDateFilter then
  begin
   ...
   if DateFilter.ItemIndex=0 then
    begin
     SQLStmt:=SQLStmt+' AND (OutDate = '''+DateToStr(dtpStartDate.Date)+''')';
     summary:=summary+' AND (OutDate = '''+DateToStr(dtpStartDate.Date)+''')';
    end
   else
    begin
     SQLStmt:=SQLStmt+' AND (InDate = '''+DateToStr(dtpStartDate.Date)+''')';
     //summary:=summary+' AND (OutDate between '''+DateToStr(dtpStartDate.Date)+''' AND '''+DateToStr(dtpEndDate.Date)+''')';
    end;
  end;
 if ReportTabs.TabIndex=0 then SQLStmt:=SQLStmt+' ORDER BY InDate desc'
 else SQLStmt:=SQLStmt+' ORDER BY OutDate desc';
...
begin
 ...
 if ReportTabs.TabIndex=0 then
  begin
   ...
   ADOQuery2.SQL.Clear;
   ADOQuery2.SQL.AddStrings(materials.SQL);
  end;
 if ReportTabs.TabIndex=1 then
  begin
   materials.SQL.Add(Stmt);
   ADOQuery1.SQL.Clear;
   ADOQuery1.SQL.AddStrings(materials.SQL);
   xlsStatement:=materials.SQL.Text;
  end;
 materials.Open;
 count:=0;
 p_key.Clear;
