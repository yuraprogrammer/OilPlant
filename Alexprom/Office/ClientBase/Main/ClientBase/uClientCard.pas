...
var
  ...
  regRegionDS, clientDS: TDataSource;
  sameAddress: boolean;
  clientData:Client;
...
begin
 ...
 if clientQuery.RecordCount<>0 then
  begin
    ...
    clientData.factAddress:=factID;
    clientData.regAddress:=regID;
  end;
end;
...
begin
 ...
 cbFactRegion.ListSource:=regRegionDS;
end;
...
begin
 ...
 sp.SQL.Add(IntToStr(clientData.regAddress)+',');
 sp.SQL.Add(IntToStr(clientData.factAddress)+',');
...
begin
 sameAddress:=cbSameAddresses.Checked;
...
begin
 Query.SQL.Clear;
 Query.SQL.Add('select * from dbo.apRegionsDic order by ID');
 Query.Open;
 regRegionDS.DataSet:=Query;
